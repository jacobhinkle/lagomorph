...
// we compute.
template<typename Real, bool displacement, BackgroundStrategy backgroundStrategy>
__global__
void
...
at::Tensor jacobian_times_vectorfield_forward(
    ...
    if (displacement) {
        ...
        jacobian_times_vectorfield_forward_kernel_2d<scalar_t, true, DEFAULT_BACKGROUND_STRATEGY><<<blocks, threads>>>(
            out.data<scalar_t>(),
            g.data<scalar_t>(),
            v.data<scalar_t>(),
            out.size(0),
        ...
        AT_DISPATCH_FLOATING_TYPES(g.type(), "jacobian_times_vectorfield_forward", ([&] {
        jacobian_times_vectorfield_forward_kernel_2d<scalar_t, false, DEFAULT_BACKGROUND_STRATEGY><<<blocks, threads>>>(
            out.data<scalar_t>(),
            g.data<scalar_t>(),
            v.data<scalar_t>(),
            out.size(0),
...
template<typename Real, bool displacement,
...
std::vector<at::Tensor> jacobian_times_vectorfield_backward(
    ...
    AT_ASSERTM(w.size(1) == dim, "wector field is of wronv dimension")
    AT_ASSERTM(!displacement || v.size(1) == dim, "Displacement mode only defined for wector fields")
    ...
    if (displacement) {
        AT_DISPATCH_FLOATING_TYPES(v.type(), "jacobian_times_wectorfield_forward", ([&] {
        jacobian_times_vectorfield_backward_kernel_2d<scalar_t, true, DEFAULT_BACKGROUND_STRATEGY, true, true><<<blocks, threads>>>(
            d_v.data<scalar_t>(),
            d_w.data<scalar_t>(),
        ...
        AT_DISPATCH_FLOATING_TYPES(v.type(), "jacobian_times_wectorfield_forward", ([&] {
        jacobian_times_vectorfield_backward_kernel_2d<scalar_t, false, DEFAULT_BACKGROUND_STRATEGY, true, true><<<blocks, threads>>>(
            d_v.data<scalar_t>(),
            d_w.data<scalar_t>(),
            grad_out.data<scalar_t>(),
            v.data<scalar_t>(),
