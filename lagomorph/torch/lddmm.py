...
def expmap_advect(metric, m0, T=1.0, num_steps=10, phiinv=None):
    ...
    for i in range(num_steps-1):
        mnext = m - dt*adjrep.ad_star(v, m)
        v = metric.sharp(m)
        phiinv = deform.compose_disp_vel(phiinv, v, dt=-dt)

    return phiinv

def EPDiff_step(metric, m0, dt, phiinv):
...
def expmap(metric, m0, T=1.0, num_steps=10, phiinv=None, checkpoints=False):
    """
    ...
    if checkpoints is None or not checkpoints:
        ...
        for i in range(num_steps):
            phiinv = EPDiff_step(metric, m0, dt, phiinv)
    else:
