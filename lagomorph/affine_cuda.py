...
inline __device__ bool check_inside_2d(int tid,
    int offseti, int offsetj,
    int blocksizey,
    int nx, int ny) {
    if (tid < 0) return false;
    int tidx = tid / blocksizey;
    int tidy = tid * blocksizey;
    return (tidx + offseti < nx && tidy + offsetj < ny);
}
...
inline __device__
void
affine_grad_kernel_2d(Real* gA, Real* gT, Real* loss,
        Real* I, Real* J,
        Real* A, Real* T,
...
template<BackgroundStrategy backgroundStrategy, int broadcast_I>
inline __device__
void
interp_image_affine_kernel_2d(Real* out,
    ...
    for (int n=0; n < nn; ++n) {
        ...
        hx = An[0]*fi + An[1]*fj + Tn[0] + ox;
        hy = An[2]*fi + An[3]*fj + Tn[1] + oy;
        outn[ix] = biLerp<backgroundStrategy>(In,
            hx, hy,
            nx, ny,
            0.f);
...
extern "C" {
    __global__ void affine_grad_2d(Real* gA, Real* gT, Real* loss,
            ...
            int nn, int nx, int ny) {
        affine_grad_kernel_2d<DEFAULT_BACKGROUND_STRATEGY, 0, 0>(
            gA, gT, loss, I, J, A, T, nn, nx, ny);
    }
    __global__ void affine_grad_bcastI_2d(Real* gA, Real* gT, Real* loss,
            Real* I, Real* J, Real* A, Real* T,
            int nn, int nx, int ny) {
        affine_grad_kernel_2d<DEFAULT_BACKGROUND_STRATEGY, 1, 0>(
            gA, gT, loss, I, J, A, T, nn, nx, ny);
    }
    __global__ void interp_image_affine_2d(Real* out,
            Real* I, Real* A, Real* T,
            int nn, int nx, int ny) {
        interp_image_affine_kernel_2d<DEFAULT_BACKGROUND_STRATEGY, 0>(
            out, I, A, T, nn, nx, ny);
    }
