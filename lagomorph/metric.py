...
class FluidMetricOperator(torch.autograd.Function):
    ...
    def forward(ctx, params, luts, inverse, mv):
        ...
        lagomorph_ext.fluid_operator(Fmv, inverse, luts["cos"], luts["sin"], *params)
        return torch.irfft(Fmv, spatial_dim, normalized=True, signal_sizes=sh[2:])
    ...
    def backward(ctx, outgrad):
        ...
        lagomorph_ext.fluid_operator(
            Fmv, ctx.inverse, ctx.luts["cos"], ctx.luts["sin"], *ctx.params
        )
        return (
            None,
            None,
            None,
            torch.irfft(Fmv, spatial_dim, normalized=True, signal_sizes=sh[2:]),
        )


class FluidMetric(object):
    def __init__(self, params=[0.1, 0.0, 0.001]):
        """
    ...
    def initialize_luts(self, shape, dtype, device="cuda"):
        ...
        if self.luts is None:
            ...
            for (Nf, N) in zip(self.complexshape[2:], self.shape[2:]):
                self.luts["cos"].append(
                    torch.Tensor(2.0 * (1.0 - np.cos(2 * np.pi * np.arange(Nf) / N)))
                    .type(dtype)
                    .to(device)
                )
                self.luts["sin"].append(
                    torch.Tensor(np.sin(2.0 * np.pi * np.arange(Nf) / N))
                    .type(dtype)
                    .to(device)
                )
...
class Metric:
    ...
    def add_args(parser):
        parser.add_argument(
            ...
            default="fluid",
            type=str,
            help="Type of parser. Currently only 'fluid' is supported.",
        )
        parser.add_argument(
            "--fluid_alpha",
            default=0.1,
            type=float,
            help="Fluid parameter for vector Laplacian term",
        )
        parser.add_argument(
            "--fluid_beta",
            default=0.0,
            type=float,
            help="Fluid parameter for gradient divergence term",
        )
        parser.add_argument(
            "--fluid_gamma",
            default=0.01,
            type=float,
            help="Fluid parameter for L2 term",
        )
    ...
    def from_args(cls, args):
        if args.metric_type.lower() == "fluid":
            return FluidMetric(
                params=[args.fluid_alpha, args.fluid_beta, args.fluid_gamma]
            )
