...
def test_affine_interp_gradcheck(bs, dim, c, testI, testA, testT):
    if not (testI or testA or testT):
    ...
    catch_gradcheck(
        f"Failed affine interp gradcheck with batch size {bs} dim {dim} channels {c}",
        lm.affine_interp,
        (I, A, T),
    )
...
def test_affine_2d_match_3d(d, bs, c):
    ...
    with torch.no_grad():
        ...
        T3 = torch.cat(
            (T2, torch.zeros((bs, 1), dtype=T2.dtype, device=I2.device)), dim=1
        )
        J2 = lm.affine_interp(I2, A2, T2).view(bs, c, res, res, 1)
        J3 = lm.affine_interp(I3, A3, T3)
        assert torch.allclose(
            J2, J3
        ), f"Failed affine 2D==3D check with batch size {bs} channels {c}"
...
def test_affine_inverse(bs, dim):
    ...
    assert torch.allclose(
        x, xhat
    ), f"Failed affine inverse with batch size {bs} dim {dim}"
