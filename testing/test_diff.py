...
def test_jacobian_times_vectorfield_gradcheck(bs, dim, disp, trans, testphi, testm):
    if not (testphi or testm):
        return  # nothing to test
    defsh = tuple([bs, dim] + [res] * dim)
    phiinv = torch.randn(defsh, dtype=torch.float64, requires_grad=testphi).cuda()
    ...
    catch_gradcheck("Failed jacobian_times_vectorfield gradcheck", foo, (phiinv, m))
...
def test_jacobian_times_vectorfield_adjoint_gradcheck(bs, dim):
    ...
    catch_gradcheck(
        "Failed jacobian_times_vectorfield_adjoint gradcheck",
...
def test_jacobian_times_vectorfield_2d_match_3d(bs, disp, trans):
    ...
    assert torch.allclose(
        dvm3[:, :2, :, :, 0], dvm2
    ), "Failed jacobian_times_vectorfield 2D match 3D"
...
def test_jacobian_times_vectorfield_adjoint_2d_match_3d(bs, disp, trans):
    ...
    assert torch.allclose(
        dvm3[:, :2, :, :, 0], dvm2
    ), "Failed jacobian_times_vectorfield_adjoint 2D match 3D"
