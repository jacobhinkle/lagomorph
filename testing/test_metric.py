...
def test_fluid_sharp_gradcheck(bs, dim):
    ...
    catch_gradcheck(
        f"Failed fluid sharp gradcheck with batch size {bs} dim {dim}",
        metric.sharp,
        (m,),
        1e-4,
    )
...
def test_fluid_flat_gradcheck(bs, dim):
    ...
    metric = lm.FluidMetric(fluid_params)
    catch_gradcheck(
        f"Failed fluid flat gradcheck with batch size {bs} dim {dim}", metric.flat, (v,)
    )
