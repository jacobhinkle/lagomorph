    ...
    for nc in [2,3]:
        for bs in batch_sizes:
            for dim in dims:
                ...
                I = torch.randn(imsh, dtype=torch.float64, requires_grad=True).cuda()
                u = torch.randn(defsh, dtype=I.dtype, requires_grad=True).to(I.device)
                catch_gradcheck(f"Failed interp gradcheck with batch size {bs} dim {dim} num channels={nc}", lt.interp, (I,u))
...
def test_interp_broadcastI_gradcheck_both():
    for bs in batch_sizes:
        for dim in dims:
            ...
            I = torch.randn(imsh, dtype=torch.float64, requires_grad=True).cuda()
            u = torch.randn(defsh, dtype=I.dtype, requires_grad=True).to(I.device)
            catch_gradcheck(f"Failed broadcastI interp gradcheck (both) with batch size {bs} dim {dim}", lt.interp, (I,u))
